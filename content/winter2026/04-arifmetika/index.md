...
publishDate: 2026-09-12T00:00:00+03:00
description: "длинные числа, умножение Карацубы, первые шаги в Git."
weight: 40
draft: false
---
...
Деление стоит особняком среди четырёх операций. У сложения и умножения каждая цифра результата — локальная формула из цифр аргументов и переноса; цифру частного так вычислить нельзя — её можно только **подобрать**: прикинуть, сколько раз делитель влезает, проверить, поправить. Школьный столбик — это дисциплина такого подбора, то есть полноценный алгоритм, а не ностальгия.

Зачем он нужен, видно от противного. Наивное деление — вычитать $b$ из $a$, пока вычитается, — корректно, но частное бывает порядка $10^n$, столько будет и итераций: экспонента от числа разрядов. Столбик вычитает не по одному $b$, а сразу $b \cdot d \cdot 10^k$ — целый разряд за шаг: шагов столько, сколько цифр в частном, и каждый дешёвый. Это та же идея, что ускоряет все остальные операции, — позиционная запись, применённая к вычитанию.

Начнём со случая, где подбор вырождается: деление длинного числа на **короткое** (влезающее в `int`) — простой проход со старших разрядов с протаскиванием остатка:
...
Строка `cur = cur*10 + a[i]` — школьное «снести следующую цифру». Трасса 8531 ÷ 7:

| снесли | cur | цифра частного | остаток |
|---|---|---|---|
| 8 | 8 | 1 | 1 |
| 5 | 15 | 2 | 1 |
| 3 | 13 | 1 | 6 |
...
Частное 1218, остаток 5; проверка: $7 \cdot 1218 + 5 = 8531$. Инвариант цикла: после обработки старшего префикса `cur` — остаток этого префикса по модулю $b$; переполнения нет, потому что `cur < b ≤ INT_MAX` и `cur*10 + цифра` заведомо влезает в `int64_t`. Этот $\Theta(n)$-проход закрывает большинство реальных нужд BigInt: поделить на 2, перевести в другую систему счисления, проверить делимость.

Деление **длинного на длинное** — школьный столбик в полный рост: на каждом шаге подбираем максимальную цифру частного $d$, при которой $b \cdot d$ ещё влезает в текущий остаток. Кандидатов всего десять, а предикат «$b \cdot d \le R$» монотонен по $d$ — подбирать можно перебором или двоичным поиском; запомните приём «подбор по монотонному предикату»: в занятии 12 он вырастет в целый метод «поиск по ответу». Итого $\Theta(n^2)$; быстрые варианты строятся через метод Ньютона (итерации для $1/b$, каждая — пара умножений) и стоят асимптотически как умножение. Краевые случаи — источник 90% багов: ноль, $b = 1$, ведущие нули результата, остаток. Пишите тесты до кода.
...
> **Тренажёр:** [learngitbranching.js.org](https://learngitbranching.js.org/?locale=ru_RU) — интерактивная песочница, которая рисует граф коммитов по мере того, как вы вводите команды. Уровни от первых коммитов до merge и rebase, есть русский язык. Полчаса в ней дают то, чего не даёт ни одна статья, — интуицию, как ветки устроены на самом деле. Пройдите первую главу перед тем, как сдавать ДЗ.
