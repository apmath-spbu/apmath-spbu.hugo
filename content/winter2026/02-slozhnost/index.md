...
description: "Фибоначчи четырьмя способами, O-символика, иерархия скоростей роста."
weight: 20
draft: false
---
...
Откуда «150 лет»: $T(90) \approx T(45) \cdot \varphi^{45}$, а $\varphi^{45} = F_{45}\sqrt{5} \approx 1.13 \cdot 10^9 \cdot 2.236 \approx 2.5 \cdot 10^9$ — сами числа Фибоначчи и подсказывают множитель. Итого $1.89 \text{ с} \times 2.5 \cdot 10^9 \approx 4.8 \cdot 10^9$ с $\approx$ 152 года.
...
Точное решение: $C(n) = 2F_{n+1} - 1$. Докажем индукцией по $n$; шаг опирается на два предыдущих значения, поэтому база — двойная.

- **База.** $C(0) = 1 = 2F_1 - 1$ и $C(1) = 1 = 2F_2 - 1$ (напомним, $F_1 = F_2 = 1$).
- **Переход** ($n \ge 2$). Пусть формула верна для $n-1$ и $n-2$. Тогда
$$C(n) = C(n-1) + C(n-2) + 1 = (2F_n - 1) + (2F_{n-1} - 1) + 1 = 2(F_n + F_{n-1}) - 1 = 2F_{n+1} - 1.$$

Единицы в переходе схлопнулись ($-1-1+1 = -1$), а рекуррента Фибоначчи доделала остальное. Проверка на дереве: $C(5) = 2F_6 - 1 = 2 \cdot 8 - 1 = 15$ — ровно столько вызовов мы насчитали на картинке. А сами числа Фибоначчи растут как $F_n = \varphi^n/\sqrt{5}$ с округлением.

Откуда взялась сама $\varphi$ — из рекурренты. Дерево растёт «в геометрической прогрессии», поэтому поищем решение прямо в таком виде: $F_n \approx c \cdot x^n$. Подстановка в $F_n = F_{n-1} + F_{n-2}$ после сокращения на $c \, x^{n-2}$ оставляет квадратное уравнение

$$x^2 = x + 1 \quad\Longrightarrow\quad x = \frac{1+\sqrt{5}}{2} = \varphi \approx 1.618$$

— единственный положительный корень, то самое золотое сечение. Экспоненту никто не приносил снаружи — она живёт внутри самой рекурренты. Строгое доказательство с обоими корнями (формула Бине) — на занятии 6 про рекуррентные соотношения. Итого время наивной рекурсии:
...
|---|---|---|---|
| $f = O(g)$ | растёт не быстрее $g$ | ≤ | $n = O(n^2)$ и $3n^2 = O(n^2)$ |
| $f = \Omega(g)$ | растёт не медленнее $g$ | ≥ | $n^2 = \Omega(n)$ и $n^2/2 = \Omega(n^2)$ |
| $f = \Theta(g)$ | тот же порядок (O и Ω сразу) | = | $5n^2 + 3n = \Theta(n^2)$ |
| $f = o(g)$ | строго медленнее: $f/g \to 0$ | < | $n = o(n^2)$, но $3n^2 \ne o(n^2)$ |
| $f = \omega(g)$ | строго быстрее: $f/g \to \infty$ | > | $n^2 = \omega(n)$, но $n^2 \ne \omega(n^2/2)$ |

Обратите внимание на вторые примеры в строках: нестрогие $O$ и $\Omega$, как ≤ и ≥, **допускают совпадение порядков** — потому $3n^2 = O(n^2)$ верно; строгие $o$ и $\omega$ его **запрещают**. На паре «$n$ и $n^2$» все четыре знака неразличимы — различие видно только на функциях одного порядка.
