...
description: "биты и байты, IEEE 754, массивы, указатели и ссылки, size_t."
weight: 30
draft: false
---
...
> **Почему оговорка «с C++20»?** До C++20 стандарт формально разрешал *три* представления знаковых целых — наследие машин 1950–60-х: прямой код (старший бит — знак, остальное — модуль; IBM 7090), обратный код (минус = инверсия всех битов; UNIVAC, CDC 6600) и дополнительный. У первых двух есть «минус ноль» и симметричный диапазон, а битовый образ, скажем, −1 в трёх кодах разный — поэтому формально он считался «implementation-defined». Машин с прямым и обратным кодом давно не существует, и C++20 (предложение [P0907 «Signed Integers are Two's Complement»](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2018/p0907r4.html)) узаконил статус-кво: знаковые целые *обязаны* быть в дополнительном коде. Всё перечисленное выше с этого момента — гарантия языка, а не свойство конкретного железа; заодно стало полностью определённым преобразование unsigned → signed вне диапазона (взятие по модулю $2^n$) и гарантировано `INT_MIN == -INT_MAX - 1`. Важно, что́ при этом **не** изменилось: переполнение знаковой арифметики осталось UB — стандарт зафиксировал представление, но не операции (об этом следующий раздел). Язык C принял то же решение только в C23.
>
> Отсюда полезная лесенка — три статуса одного и того же поведения. **Де-факто:** так делает всё реальное железо (x86, ARM, RISC-V) и все компиляторы — программа 2005 года исполнялась байт в байт так же, как сегодня. **Implementation-defined:** статус до C++20 — компилятор обязан задокументировать своё поведение, но язык его не обещает (это не UB, «смотри документацию платформы»). **Гарантия стандарта:** статус с C++20 — обещание самого языка, на которое можно опираться в переносимом коде. P0907 не изменил поведение ни одной машины — он передвинул его по лесенке с середины на верхнюю ступень.
