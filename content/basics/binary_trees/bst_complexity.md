...
Высота BST критически влияет на эффективность всех [операций]({{% relref "basics/binary_trees/bst_operations" %}}). В зависимости от порядка вставки ключей, высота может варьироваться от логарифмической до линейной. Для гарантированной логарифмической высоты используются [АВЛ-деревья]({{% relref "basics/binary_trees/avl_trees" %}}) и [красно-чёрные деревья]({{% relref "basics/binary_trees/red_black_trees" %}}).
