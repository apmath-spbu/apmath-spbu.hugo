...
Красно-чёрное дерево (Red-Black Tree, RB-tree) — это самобалансирующееся двоичное дерево поиска, которое обеспечивает логарифмическую высоту с меньшим количеством вращений, чем [АВЛ-дерево]({{% relref "basics/binary_trees/avl_trees" %}}). Как и АВЛ-дерево, оно основано на [базовых операциях BST]({{% relref "basics/binary_trees/bst_operations" %}}).
