...
Также можно оценить время работы нашего алгоритма с помощью [основной теоремы о рекуррентных соотношениях]({{% relref "basics/recurrence_relation" %}}): время работы алгоритма в худшем случае (когда $x$ так и не нашёлся, либо нашёлся в самом конце) удовлетворяет рекуррентному соотношению $T(n)=T\left(\frac{n}{2}\right)+\Theta(1)$. Это соответствует $a=1, b=2, c=0$ в формулировке теоремы. $c=0=\log _{2} 1=\log _{b} a$, тогда по теореме время работы алгоритма в худшем случае есть $\Theta(\log n)$.
...
В C++ есть встроенные функции [std::lower_bound](https://en.cppreference.com/w/cpp/algorithm/lower_bound.html) и [std::upper_bound](https://en.cppreference.com/w/cpp/algorithm/upper_bound.html). Первая возвращает итератор, указывающий на первый элемент, больше или равный $x$, вторая на первый элемент, строго больший $x$. Пример использования:
