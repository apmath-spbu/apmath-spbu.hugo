---
title: "13.3. Графы. Алгоритмы поиска кратчайших путей"
date: 2026-04-12T09:30:00+03:00
weight: 133
draft: false
---

Во [взвешенном графе]({{% relref "basics/graphs/graph_definition" %}}) (weighted graph) каждое ребро имеет вес (weight) $w_{e}$. В зависимости от задачи, веса могут быть как целыми, так и вещественными; иногда допускаются отрицательные веса.
...
Если реализовать очередь с приоритетами с помощью [двоичной кучи]({{% relref "basics/heap" %}}), то получим время работы $O(V \cdot \log V+E \cdot \log V)=O((V+E) \log V)$.
...
Если известно, что веса рёбер и длины путей - целые числа, не превосходящие $C$, реализация очереди с приоритетами с помощью [дерева Ван-Эмде-Боаса]({{% relref "basics/hashing" %}}) даст оценку времени работы $O((V+E) \log \log C)$.
...
| Алгоритм | Задача | Время | Ограничения на веса |
|----------|--------|-------|---------------------|
| BFS | $s \to$ все | $O(V + E)$ | Невзвешенный (все веса = 1) |
| 0-1-BFS | $s \to$ все | $O(V + E)$ | Веса $\in \{0, 1\}$ |
| 1-$k$-BFS (способ 1) | $s \to$ все | $O(V + kE)$ | Целые веса $\in [1, k]$ |
| 1-$k$-BFS (способ 2) | $s \to$ все | $O(kV + E)$ | Целые веса $\in [1, k]$ |
| Дейкстра (массив) | $s \to$ все | $O(V^2 + E)$ | Веса $\geqslant 0$ |
| Дейкстра (бин. куча) | $s \to$ все | $O((V + E) \log V)$ | Веса $\geqslant 0$ |
| Дейкстра (фиб. куча) | $s \to$ все | $O(V \log V + E)$ | Веса $\geqslant 0$ |
| $A^*$ | $s \to t$ | $\leqslant$ Дейкстра | Веса $\geqslant 0$, нужна эвристика $f$ |
| Форд-Беллман | $s \to$ все | $O(VE)$ | Любые, без отрицательных циклов |
| SPFA | $s \to$ все | $O(VE)$, часто быстрее | Любые, без отрицательных циклов |
| Флойд-Уоршелл | Все $\to$ все | $O(V^3)$ | Любые, без отрицательных циклов |
| $V \times$ Дейкстра | Все $\to$ все | $O(V^2 \log V + VE)$ | Веса $\geqslant 0$ |
| Джонсон | Все $\to$ все | $O(V^2 \log V + VE)$ | Любые, без отрицательных циклов |

### Выбор алгоритма

- **Невзвешенный граф** $\to$ BFS
- **Неотрицательные веса, одна вершина** $\to$ Дейкстра (куча для разреженных, массив для плотных)
- **Неотрицательные веса, до конкретной вершины** $\to$ A* (при наличии хорошей эвристики)
- **Отрицательные веса, одна вершина** $\to$ Форд-Беллман или SPFA
- **Нужно проверить наличие отрицательного цикла** $\to$ Форд-Беллман ($V$-й шаг)
- **Все пары, неотрицательные веса** $\to$ $V \times$ Дейкстра
- **Все пары, произвольные веса** $\to$ Флойд (простой), Джонсон (быстрее на разреженных)
- **Веса 0/1** $\to$ 0-1-BFS
- **Малые целые веса $[1, k]$** $\to$ 1-$k$-BFS
