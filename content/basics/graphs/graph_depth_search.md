---
title: "13.2. Графы. Поиск в глубину"
...
Большая часть рассуждений применима как к ориентированным, так и к неориентированным [графам]({{% relref "basics/graphs/graph_definition" %}}) (если не сказано обратное); допускаются кратные рёбра и петли (если не сказано обратное). Предполагается хранение графа [списками смежности]({{% relref "basics/graphs/graph_definition" %}}).
...
def dfs(v):
    visited[v] = True
    for u in adj[v]:  # adj[v] — список смежности вершины v
        if not visited[u]:
            dfs(u)
...
Итак, $H$ - связный граф без циклов, то есть дерево. Если ориентировать рёбра этого дерева, как сказано выше, получится корневое дерево (корень - вершина $v$ ). Мы снова будем пользоваться терминами, которыми пользовались при изучении [двоичных деревьев]({{% relref "basics/binary_trees/binary_tree_basics" %}}): ребёнок, родитель, потомок, предок, лист и так далее.
...
{{% tab color="blue" title="python" %}}
```python
# top_order — список вершин в порядке убывания t_out
# adj_t — списки смежности транспонированного графа
components = []
visited = [False] * n

def dfs_t(v, component):
    visited[v] = True
    component.append(v)
    for u in adj_t[v]:
        if not visited[u]:
...
{{% tab color="blue" title="c++" %}}
```cpp
// topOrder — список вершин в порядке убывания t_out
// adjT — списки смежности транспонированного графа
vector<vector<int>> components;
vector<bool> visited(n, false);

void dfsT(int v, vector<int>& component) {
    visited[v] = true;
    component.push_back(v);
    for (int u : adjT[v]) {
        if (!visited[u]) {
...
def dfs(v, parent):
    ...
    for u in adj[v]:
        if u != parent:
            if not visited[u]:
                dfs(u, v)
                if low[u] > tin[v]:  # (v, u) — мост
                    component = []
                    while not component or component[-1] != u:
...
for v in range(n):
    if not visited[v]:
        stack.clear()
        dfs(v, -1)
        components.append(list(stack))  # компонента корня
```
{{% /tab %}}
{{% tab color="blue" title="c++" %}}
```cpp
        ...
        if (u != parent) {
            if (!visited[u]) {
                ...
                if (low[u] > tin[v]) { // (v, u) — мост
                    ...
                    while (component.empty() || component.back() != u) {
...
for (int v = 0; v < n; v++) {
    if (!visited[v]) {
        stk.clear();
        dfs(v, -1);
        components.push_back(stk); // компонента корня
    }
}
```
{{% /tab %}}
...
{{% tab color="blue" title="python" %}}
```python
# Для удобства считаем граф простым
T = 0
tin = [0] * n
low = [0] * n
visited = [False] * n
edge_stack = []   # стек рёбер
components = []   # компоненты двусвязности (списки рёбер)
...
def dfs(v, parent):
    ...
    for u in adj[v]:
        if u != parent:
            if not visited[u] or tin[u] < tin[v]:
                edge_stack.append((v, u))
            if not visited[u]:
                count += 1
                dfs(u, v)
                if low[u] >= tin[v]:  # (v, u) — интересное ребро
                    component = []
                    while not component or component[-1] != (v, u):
                        component.append(edge_stack.pop())
                    components.append(component)
                    if parent != -1 or count >= 2:
                        pass  # v — точка сочленения
                low[v] = min(low[v], low[u])
            else:  # обратное ребро
                low[v] = min(low[v], tin[u])
        ...
        if (u != parent) {
            ...
            if (!visited[u]) {
                ...
                if (low[u] >= tin[v]) { // (v, u) — интересное
                    ...
                    while (component.empty() || component.back() != make_pair(v, u)) {
                        component.push_back(edgeStack.back());
                        edgeStack.pop_back();
                    }
                    components.push_back(component);
                    if (parent != -1 || count >= 2) {
