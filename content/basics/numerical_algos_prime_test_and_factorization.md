...
def linear_sieve(n):
    ...
    d = [-1] * (n + 1)  # d[i] - номер минимального простого делителя i в списке primes
    
    for i in range(2, n + 1):
        if d[i] == -1:  # i - простое число
            d[i] = len(primes)
            primes.append(i)
        
        j = 0
        while j <= d[i] and i * primes[j] <= n:
            d[i * primes[j]] = j
            j += 1
...
std::vector<int> linear_sieve(int n) {
    ...
    std::vector<int> d(n + 1, -1);  // d[i] - номер минимального простого делителя i в списке primes
...
def FermatTest(n, s):
    for i in range(s):
        a = random.randint(1, n - 1)  # случайное число от 1 до n - 1
        if pow(a, n - 1, n) != 1:     # modExp через встроенный pow с модулем
...
long long modExp(long long a, long long power, long long mod) {
    long long result = 1;
    a = a % mod;
...
// возвращает false, если n составное;
// возвращает true, если все проверки пройдены успешно
bool FermatTest(long long n, int s) {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<long long> dist(1, n - 1);
    
    for (int i = 0; i < s; i++) {
        long long a = dist(gen);  // случайное число от 1 до n - 1
        if (modExp(a, n - 1, n) != 1) {
            return false;  // n точно составное
        }
    }
...
>[!lemma]Лемма о количестве свидетелей Ферма
...
def modExp(a, m, n):
    """Быстрое возведение в степень по модулю: a^m mod n"""
    result = 1
    a = a % n
    while m > 0:
        if m & 1:  # если m нечетно
            result = (result * a) % n
        a = (a * a) % n
        m >>= 1
    return result
...
def MillerRabinTest(n, s=10):
    ...
    m = n - 1
    k = 0
    # найдём k, m такие, что n - 1 = m * 2 ** k
    while m % 2 == 0:
        m //= 2
        k += 1
...
// Быстрое возведение в степень по модулю: a^m mod n
long long modExp(long long a, long long m, long long n) {
    long long result = 1;
    a = a % n;
    while (m > 0) {
        if (m & 1) {  // если m нечетно
...
// возвращает false, если n составное;
// возвращает true, если все проверки пройдены успешно
bool MillerRabinTest(long long n, int s = 10) {
    if (n < 2) return false;
...
>>[!prove]+
>>Мы можем считать, что $n$ - число Кармайкла, так для остальных $n$ утверждение теоремы следует из леммы "О количестве свидетелей Ферма". Рассмотрим два случая - либо $n=p^{t}$ для некоторого простого $p$ и $t \geqslant 2$, либо $n=n_{1} n_{2}$ для некоторых взаимно простых $n_{1}>1$ и $n_{2}>1$.
>>
...
long long rhoAlgorithm(long long n) {
    // случайное число от 0 до n - 1
    long long y = rand() % n;
    long long x = y;
    long long d = 1;
...
int main() {
    srand(time(nullptr));
    long long n = 8051; // пример составного числа
    std::cout << "Делитель: " << rhoAlgorithm(n) << std::endl;
    return 0;
}
```
{{% /tab %}}
...
✍️ Существуют и более быстрые алгоритмы факторизации. Например, время работы алгоритма [GNFS](https://en.wikipedia.org/wiki/General_number_field_sieve) (General Number Field Sieve) эвристически оценивается как $O\left(e^{(64 / 9)^{1 / 3}(\log n)^{1 / 3}(\log \log n)^{2 / 3}}\right)$.
