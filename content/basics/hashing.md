---
title: "8. Хеширование"
date: 2026-03-13T09:30:00+03:00
draft: false
weight: 80
---
...
class HashTable:
    ...
    def find(self, x):
        """Возвращает True, если x лежит в хеш-таблице"""
        p = self.h(x)
        for i in range(len(self.T[p])):
            if self.T[p][i] == x:
                return True
        return False
    
    def insert(self, x):
        """Вставляет x в хеш-таблицу"""
        if not self.find(x):
            p = self.h(x)
            self.T[p].append(x)
    
    def delete(self, x):
        """Удаляет x из хеш-таблицы"""
        p = self.h(x)
        for i in range(len(self.T[p])):
            if self.T[p][i] == x:
                # Обмениваем с последним элементом и удаляем
                self.T[p][i], self.T[p][-1] = self.T[p][-1], self.T[p][i]
                self.T[p].pop()
                break
...
ht = HashTable(10, hash_func)
ht.insert(5)
ht.insert(15)
ht.insert(25)
print(ht.find(15))  # True
ht.delete(15)
print(ht.find(15))  # False
```
{{% /tab %}}
...
public:
    HashTable(int size, std::function<int(int)> hash_func) : m(size), h(hash_func) {
        T.resize(m);
    }
...
int main() {
    // Простая хеш-функция
    auto hash_func = [](int x) { return x % 10; };
    
    HashTable ht(10, hash_func);
    ht.insert(5);
    ht.insert(15);
    ht.insert(25);
    
    std::cout << std::boolalpha << ht.find(15) << std::endl;  // true
    ht.remove(15);
    std::cout << ht.find(15) << std::endl;  // false
...
def getIndex(x):
    # возвращает индекс ячейки, в которой лежит x,
...
// Вспомогательная функция хеширования (нужно определить)
int h(int key, int i);
